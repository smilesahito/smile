...
************************************************************************************/
		...
		public static function GetBP($caretaker_id=""){
			...
			where u.status='A' and u.user_id in (
			SELECT u.user_id FROM user u INNER JOIN patient_caretaker ck ON u.user_id = ck.patient_id WHERE ck.caretaker_id=".$caretaker_id.")";
			//echo $query; die;
...
/************************** View Caretaker Patients *****

************************************************************************************/

		public static function GetCaretakerPatients($caretaker_id=""){

			global $config;

			$dbconn=db::singleton();
			
			$query = "SELECT u.user_id, u.*
						FROM user u INNER JOIN patient_caretaker ck ON u.user_id = ck.patient_id
						WHERE ck.caretaker_id =".$caretaker_id;
			//echo $query; die;				
			$dbconn->SetQuery($query);
...
************************************************************************************/
		...
		public static function GetPrescriptionList($caretaker_id=""){

			global $config;

			$dbconn=db::singleton();
			$query = "select u.user_id, u.first_name, u.last_name, pres.* from user u INNER JOIN prescription pres ON u.user_id = pres.doctor_id WHERE pres.patient_id in (
SELECT u.user_id FROM user u INNER JOIN patient_caretaker ck ON u.user_id = ck.patient_id WHERE ck.caretaker_id=".$caretaker_id.")";
...
************************************************************************************/
		...
		public static function GetMedicalRecord($caretaker_id=""){

			global $config;

			$dbconn=db::singleton();
			$query = "select u.user_id, concat(u.first_name,' ', u.last_name) name, mr.* from user u INNER JOIN medical_records mr ON u.user_id = mr.recommended_by WHERE mr.patient_id in (
SELECT u.user_id FROM user u INNER JOIN patient_caretaker ck ON u.user_id = ck.patient_id WHERE ck.caretaker_id=".$caretaker_id.")";
